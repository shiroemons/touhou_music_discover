  ...
  class Album
    ...
    class NativeBackend
      ...
      def self.process_album(s_album)
        spotify_album = resolve_spotify_album(s_album)
        return if spotify_album.nil? || spotify_album.total_tracks == spotify_album.spotify_tracks.count

        simplified_tracks = fetch_all_tracks(spotify_album.spotify_id)
      ...
      def self.resolve_spotify_album(s_album)
        ...
        SpotifyAlbum.save_album(full_album)
      end
      private_class_method :resolve_spotify_album

      # GET /albums/{id} に埋め込まれた tracks は market を指定していなくても track
      # relinking の対象になり、id が市場別の relinked ID になる (linked_from.id が
      # 真の canonical ID を指す)。GET /albums/{id}/tracks (専用エンドポイント) は
      # relinking されず canonical ID を返す。既存の spotify_tracks.spotify_id は
      # canonical ID で保存されているため、埋め込みを再利用すると既存トラックとの照合に
      # 失敗し、同じ曲が重複保存される。だから 1 リクエスト節約のためだけに埋め込みを
      # 再利用してはならない。
      def self.fetch_all_tracks(spotify_id)
        items = []
        page = SpotifyApi::Album.tracks(spotify_id, limit: LIMIT, offset: 0)
        loop do
